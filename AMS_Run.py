...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        # Start a separate thread to create the WebView
        threading.Thread(target=self.create_webview).start()

    def create_webview(self):
        # Create a WebView to display the HTML login page
        webview.create_window("Login Page", url="index.html", width=800, height=600, js_api=self.handle_js_message)

    def handle_js_message(self, message):
        # Handle messages received from JavaScript
        if message == "login_success":
            # Close the WebView and show the main Tkinter window
            webview.destroy_window("Login Page")
            self.show_main_window()

    def show_main_window(self):
        # Display the main Tkinter window after successful login
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'Attendace managemnt system\Attendance\\Manually Attendance\\-------Check atttendance-------"')
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='snow')
                cs = r"C:\Users\PARVINKUMAR\OneDrive\Desktop\Face recognition\Attendace_management_system\Attendance\php_2024-03-13_23-03-03.csv"
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"Attendace managemnt system\Attendance\-------Check atttendance-------"')

    attf = tk.Button(windo,  text="Check Sheets",command=Attf,fg="black"  ,bg="red"  ,width=12  ,height=1 ,activebackground = "Red" ,font=('times', 14, ' bold '))
    ...
    def log_in():
        ...
        if username == 'kushal' :
            if password == 'kushal14320':
                ...
                cs = 'Attendace managemnt system/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
lbl = tk.Label(window, text="Enter Enrollment", width=20, height=2, fg="#F3F8FF", bg="#000000", font=('times', 15, ' bold '))
...
txt = tk.Entry(window, validate="key", width=20, bg="#FF8400", fg="red", font=('times', 25, ' bold '))
txt['validatecommand'] = (txt.register(testVal),'%P','%d')
txt.place(x=550, y=210)

lbl2 = tk.Label(window, text="Enter Name", width=20, fg="#F3F8FF", bg="#0096FF", height=2, font=('times', 15, ' bold '))
lbl2.place(x=200, y=300)

txt2 = tk.Entry(window, width=20, bg="#FF8400", fg="red", font=('times', 25, ' bold '))
